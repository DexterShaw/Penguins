...
void BoardGeneration(char* mode, int nplayers); //generates the board
void ArgumnetHandling(int number, char** console);
struct board** LoadBoard(); //reads the board from the file and returns it
...
int Menu(); //menu
void OverwriteFile(struct board** brd, char *boardname); //
...
struct board** LoadBoard(char *boardname)
{
	int i, j;
	int x=0, y=0;
	char dim[1024];
	...
	for (i = 0; i<l1z; i++)
	{
		y = y + (dimy[i] - '0') * pow(10, l1 - 1);
		l1--;
	}
	for (i = 0; i<l2z; i++)
	{
		x = x + (dimx[i] - '0') * pow(10, l2 - 1);
		l2--;
	}
...
void PrintBoard(struct board** brd, int turn, int nplayers)
{
	...
	{
		printf("Press \"x\" to place a penguin\n\n");
	}
	if (strcmp(set1.phase,"movement")==0)
	{
		printf("Press \"e\" to select the penguin and \"r\" where you want to move it\n\n");
	}
	...
	for (i = 0; i < set1.dimy + 2; i++)
	{
		for (j = 0; j < set1.dimx + 2; j++)
		{
			if(brd[i][j].cursor==1)
			{
				if(brd[i][j].fish==0 && brd[i][j].playerid==0)
				{
					printf("Currently selected tile: X \n\n");
				}
				if(brd[i][j].fish==0 && brd[i][j].playerid!=0)
				{
					printf("Currently selected tile: P-%d\n\n", brd[i][j].playerid);
				}
				if(brd[i][j].fish!=0 && brd[i][j].playerid==0)
				{
					printf("Currently selected tile: %d\n\n", brd[i][j].fish);
				}
			}
		}
	}
...
struct board** AutonomousPlacement(struct board** brd, int playernumber)
{
		...
		brd[ry][rx].playerid = playernumber;
		brd[ry][rx].fish = 0;
		p[playernumber-1].score = p[playernumber-1].score + 1;
...
struct board** AutonomousMovement(struct board** brd, int playernumber)
{
	int i, j, k = 0;
	...
	for (i = 1; i < set1.dimy + 2 - 1; i++)
	{
		for (j = 1; j < set1.dimx + 2 - 1; j++)
		{
			if (brd[i][j].playerid == playernumber)
			{
				npenguins++;
			}
		}
	...
	{
		if(Nblocked(brd, n[i], m[i]) == 4)
		{
			blocked++;
		}
	}
	if(blocked==npenguins)
	{
		return brd;
	}
	...
	while (brk==0)
	{
		...
		if (r2 == 1)
		{
			for (i = n[r] - 1; brd[i][m[r]].fish != 0 && brd[i][m[r]].border != 1; i--)
			{
				...
				{
					brd[n[r]][m[r]].playerid = 0;
					brd[i][m[r]].playerid = playernumber;
					p[playernumber-1].score = p[playernumber-1].score + brd[i][m[r]].fish;
					brd[i][m[r]].fish = 0;
					brk = 1;
					break;
				}
		...
		if (r2 == 3)
		{
			for (i = m[r] - 1; brd[n[r]][i].fish != 0 && brd[n[r]][i].border != 1; i--)
			{
				...
				{
					brd[n[r]][m[r]].playerid = 0;
					brd[n[r]][i].playerid = playernumber;
					p[playernumber-1].score = p[playernumber-1].score + brd[n[r]][i].fish;
					brd[n[r]][i].fish = 0;
					brk = 1;
					break;
				}
