...
struct board** LoadBoardAuto(char *boardname);
...
int Menu(); //menu
void OverwriteFile(struct board** brd, int nplayers, char *boardname); //
...
struct board** CompPlacement(struct board** brd);
...
struct board** LoadBoard(char *boardname)
{
	...
	for (i = 0; dim[i] != ' '; i++)
	{
		y = y + (dim[i] - '0') * pow(10, l1 - 1);
		l1--;
	}
	for (i = i + 1; dim[i] != '\n'; i++)
	{
		x = x + (dim[i] - '0') * pow(10, l2 - 1);
		l2--;
	}
...
struct board** LoadBoardAuto(char *boardname)
{
	...
	FILE* file;
	file = fopen(boardname, "r");
	if(file==NULL) printf("file not there \n");
	int y=0, x=0, k=0, i=0, lp=0, rp=0;
	char a, b, c;
	

	fscanf(file, "%d %d ", &y, &x);	//taking height and width from file
	
	
		brd = (struct board**)malloc((y+1) * sizeof(struct board*));
	for (i = 0; i < y+1; i++)
	{
		brd[i] = (struct board*)malloc((x+1) * sizeof(struct board));
	}
	
	
	set1.dimx = x;
	set1.dimy = y;
	//fscanf(file, "%c", &c); //taking an empty character just because
	printf("%d %d \n", set1.dimy, set1.dimx);
	...
	printf("\n");
	fscanf(file, "%c", &c);
	if(fscanf(file, "%s ", &p[0].name)==1){
		fscanf(file, "%d ", &p[0].id);
		fscanf(file, "%d ", &p[0].score);
		set1.numofplayers+=1;
	}else set1.id=1;
...
{
	srand(time(0));
	int y, x, i=0, lx, ux, ly, uy,xpos, ypos, count=0 ;
	y=set1.dimy;
	x=set1.dimx;
	lx=(x/2)-x/4;
	ux=(x/2)+x/4;
	...
	while(i==0&&count<((ux-lx)*(uy-ly))){
		xpos=(rand()%(ux-lx+1))+lx;
		ypos=(rand()%(uy-ly+1))+ly;
		if((brd[ypos][xpos].fish==1)&&(brd[ypos][xpos].playerid==0)){
			brd[ypos][xpos].fish==0;
			brd[ypos][xpos].playerid=set1.id;
			set1.score+=1;
			i=1;
		}
		count++;
...
void PrintBoardAuto(struct board** brd){
	int i, k;
	printf("%d %d \n", set1.dimy, set1.dimx);
	
	for(i=0; i<set1.dimy; i++){
		for(k=0; k<set1.dimx; k++){
			printf("%d" , brd[i][k].playerid);
			printf("%d" , brd[i][k].fish);
			printf(" ");
		}
		printf("\n");
	}
